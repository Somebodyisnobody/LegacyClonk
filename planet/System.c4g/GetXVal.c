...
// *** einige Funktionen, die Informationen �ber die
//     GetXVal-Familie abfragen

// Achtung: Diese Funktionen greifen direkt auf interne Werte der Engine zu. Die Benutzung
//          kann unter Umst�nden zu Desynchronisation in Netzwerkspielen oder aufgenommenen
//          Spielen f�hren. Einige besonders gef�hrliche Funktionen sind im Folgenden mit
//          "/*(!)*/" gekennzeichnet.
...
global func GetDefLineConnect(id def) { return(GetDefCoreVal("LineConnect", "DefCore", def)); }
global func GetDefExclusive(id def)   { return(GetDefCoreVal("Exclusive", "DefCore", def)); }

...
Read::Read(const std::string &filename)
{
	...
	if (!file)
	{
		throw Exception(std::string{"Opening \""} + filename + "\": " + std::strerror(errno));
	}
...
Write::Write(const std::string &filename)
{
	...
	if (const auto ret = deflateInit2(&gzStream, 0, Z_DEFLATED, 15 + 16, 9, Z_DEFAULT_STRATEGY); ret != Z_OK)
	{
		fclose(file);
